...
from labelFile import LabelFileError
from lib import addActions
from lib import fmtShortcut
from lib import newAction
...
class MainWindow(QtWidgets.QMainWindow, WindowMixin):
    ...
    def __init__(self, config=None, filename=None, output=None):
        ...
        delete_img = action('&Delete Image', self.deleteImage,
                            shortcuts['delete_polygon'], 'delete',
                            'Delelte image and label', enabled=True)

        track_img = action('&Track Image', self.trackImage,
                            shortcuts['delete_polygon'], 'eye',
                            'Track image path', enabled=True)

        save = action('&Save', self.saveFile, shortcuts['save'], 'save',
                      'Save labels to file', enabled=True)
        ...
        addActions(self.menus.file, (open_, delete_img,self.menus.recentFiles,
    ...
    def saveLabels(self, filename):
        ...
        try:
            ...
            cf = self.labelFile
            self.labelFile = lf
            self.labelFile.labelUserName = cf.labelUserName
            self.labelFile.qualityUserName = cf.qualityUserName
            self.labelFile.secondQualityUserName = cf.secondQualityUserName
            self.labelFile.companyName = cf.companyName
            self.labelFile.title = cf.title
    ...
    def trackImage(self,_value=False):
        txt = "文件名：" + self.labelFile.filename + '\n'
        txt += "标注员：" + self.labelFile.labelUserName.encode("utf8") + '\n'
        txt += '质检员：' + self.labelFile.qualityUserName.encode("utf8") + '\n'
        txt += '二次质检员：' + self.labelFile.secondQualityUserName.encode("utf8") + '\n'
        txt += '公司：' + self.labelFile.companyName.encode("utf8") + '\n'
        txt += '标题：' + self.labelFile.title.encode("utf8") + '\n'
        #g.textbox(msg='标注信息：', title='文件', text=txt, codebox=1)
        # xml_file, ext = os.path.basename(self.imagePath).split('.')
        # xml_file += '.xml'
        # for root,dirs,files in os.walk(xml_original_dir):
        #     for f in files:
        #         if(f==xml_file):
        #             print(os.path.join(root,f))

    def deleteImage(self, _value=False):
        if not self.image.isNull():
            if not os.path.exists(delete_dir):
                os.makedirs(delete_dir)
            if self.labelFile is not None and os.path.exists(self.labelFile.filename):
                shutil.move(self.labelFile.filename,delete_dir)
                print("file : "+self.labelFile.filename)
            fname = os.path.basename(self.labelFile.filename)
            filename, ext = fname.split('.')
            p = os.path.dirname(self.labelFile.filename)
            jpg_file = p+'/'+filename+'.jpg'
            jpeg_file = p+'/'+filename+'.JPG'
            if os.path.exists(self.imagePath):
                shutil.move(self.imagePath,delete_dir)
                print("jpg : "+self.imagePath)
            else:
                if os.path.exists(jpg_file):
                    shutil.move(jpg_file, delete_dir)
                    print("jpg : " + jpg_file)
                elif os.path.exists(jpeg_file):
                    shutil.move(jpeg_file, delete_dir)
                    print("JPG : " + jpg_file)
            xml_file,ext = self.imagePath.split('.')
            xml_file += '.xml'
            if os.path.exists(xml_file):
                shutil.move(xml_file,delete_dir)
                print(xml_file)
            items = self.fileListWidget.selectedItems()
            if not items:
                return
...
'''
1. add method deleteImage()
2. add method trackImage() 
'''
root_dir = '/media/ubuntu/b8f80802-d95a-41c3-b157-6f4e34967425/data-banghao/2019-1-17-whole-4'
data_dir = '/home/ubutnu/data/chenjianfeng'
delete_dir= '/home/ubutnu/data/chenjf_del'
# track image original path
xml_original_dir=data_dir
